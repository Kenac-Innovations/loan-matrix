-- Preview delete script for Rulethu leads in loan_matrix_uat.
-- Host requested by user: 10.10.0.24
-- Suggested run command:
--   PGPASSWORD='***' psql -h 10.10.0.24 -p 30544 -U app -d loan_matrix_uat -f scripts/rulethu_loan_matrix_lead_delete_preview.sql
--
-- This script is intentionally left in preview mode with ROLLBACK at the end.
-- After review, replace ROLLBACK with COMMIT.
-- Matching scope is intentionally limited to known Fineract client IDs only.
...
WHERE "tenantId" = 'cmh607k3d0000vc0k5xxjocju'
  AND "fineractClientId" IN (5, 134, 135, 136, 137, 138, 139, 140, 141, 142, 143, 144, 145, 146);
...
WHERE "tenantId" = 'cmh607k3d0000vc0k5xxjocju'
  AND (
    "fineractClientId" IN (
      SELECT DISTINCT fineract_client_id
      FROM target_leads
