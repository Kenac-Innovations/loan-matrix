...
async function testDatabaseRetrieval() {
  ...
  try {
    ...
    console.log(`✅ Retrieved ${ussdLeadsData.applications.length} USSD applications`);
...
async function main() {
  ...
  console.log('========================');
  console.log(`Queue Connection: ${results.queueConnection ? '✅ PASS' : '❌ FAIL'}`);
  console.log(`Message Publishing: ${results.messagePublishing ? '✅ PASS' : '❌ FAIL'}`);
  console.log(`Database Retrieval: ${results.databaseRetrieval ? '✅ PASS' : '❌ FAIL'}`);
