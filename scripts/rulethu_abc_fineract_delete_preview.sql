-- Preview delete script for Rulethu client ABC in fineract_tenant_rulethu.
-- Host requested by user: 10.10.0.24
-- Suggested run command:
--   PGPASSWORD='***' psql -h 10.10.0.24 -p 30544 -U app -d fineract_tenant_rulethu -f scripts/rulethu_abc_fineract_delete_preview.sql
--
-- Target client:
--   94 = ABC
--
...
INSERT INTO target_clients (id) VALUES (94);
...
WHERE from_loan_transaction_id IN (SELECT id FROM target_loan_txns)
   OR to_loan_transaction_id IN (SELECT id FROM target_loan_txns)
   OR to_loan_charge_id IN (SELECT id FROM target_loan_charges);
