...
export interface DownloadClientDocumentResult {
  success: boolean;
  fileName?: string;
  contentType?: string;
...
export async function downloadClientDocumentAttachment(
  clientId: number | string,
  documentId: number | string
): Promise<DownloadClientDocumentResult> {
  try {
    ...
    const url = `${baseUrl.replace(
      /\/$/,
      ""
    )}/fineract-provider/api/v1/clients/${clientId}/documents/${documentId}/attachment`;
