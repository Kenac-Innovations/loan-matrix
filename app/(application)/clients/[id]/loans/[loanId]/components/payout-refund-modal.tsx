...
interface PayoutRefundTemplate {
  type: {
    id: number;
    code: string;
    value: string;
...
interface PayoutRefundModalProps {
  isOpen: boolean;
  onClose: () => void;
  loanId: number;
  onSuccess: () => void;
...
export function PayoutRefundModal({ isOpen, onClose, loanId, onSuccess }: PayoutRefundModalProps) {
  const [template, setTemplate] = useState<PayoutRefundTemplate | null>(null);
  ...
  const fetchTemplate = async () => {
    try {
      ...
      console.error("Error fetching payout refund template:", err);
      setError(err instanceof Error ? err.message : "Failed to load payout refund template");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      console.error("Error submitting payout refund:", err);
      setError(err instanceof Error ? err.message : "Failed to submit payout refund");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    return (
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>Payout Refund</DialogTitle>
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Record a payout refund for loan #{loanId}
          </DialogDescription>
        </DialogHeader>
        ...
        {success && (
          <Alert className="border-green-200 bg-green-50">
            ...
            <AlertDescription className="text-green-800">
              Payout Refund submitted successfully!
            </AlertDescription>
          </Alert>
        )}
