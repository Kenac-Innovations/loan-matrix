...
interface InterestPaymentWaiverTemplate {
  type: {
    id: number;
    code: string;
    value: string;
...
interface InterestPaymentWaiverModalProps {
  isOpen: boolean;
  onClose: () => void;
  loanId: number;
  onSuccess: () => void;
...
export function InterestPaymentWaiverModal({ isOpen, onClose, loanId, onSuccess }: InterestPaymentWaiverModalProps) {
  const [template, setTemplate] = useState<InterestPaymentWaiverTemplate | null>(null);
  ...
  const fetchTemplate = async () => {
    try {
      ...
      const response = await fetch(`/api/fineract/loans/${loanId}/transactions/interest-payment-waiver-template`);
      if (!response.ok) {
        throw new Error(`Failed to fetch interest payment waiver template: ${response.statusText}`);
      }
      ...
      console.error("Error fetching interest payment waiver template:", err);
      setError(err instanceof Error ? err.message : "Failed to load interest payment waiver template");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await fetch(`/api/fineract/loans/${loanId}/transactions/interest-payment-waiver`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.defaultUserMessage || errorData.error || `Failed to submit interest payment waiver: ${response.statusText}`);
      }

      const result = await response.json();
      console.log("Interest payment waiver submitted successfully:", result);
      ...
      console.error("Error submitting interest payment waiver:", err);
      setError(err instanceof Error ? err.message : "Failed to submit interest payment waiver");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    return (
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>Interest Payment Waiver</DialogTitle>
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Record an interest payment waiver for loan #{loanId}
          </DialogDescription>
        </DialogHeader>
        ...
        {success && (
          <Alert className="border-green-200 bg-green-50">
            ...
            <AlertDescription className="text-green-800">
              Interest Payment Waiver submitted successfully!
            </AlertDescription>
          </Alert>
        )}
