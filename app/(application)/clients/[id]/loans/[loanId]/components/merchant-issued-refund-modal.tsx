...
interface MerchantIssuedRefundTemplate {
  type: {
    id: number;
    code: string;
    value: string;
...
interface MerchantIssuedRefundModalProps {
  isOpen: boolean;
  onClose: () => void;
  loanId: number;
  onSuccess: () => void;
...
export function MerchantIssuedRefundModal({ isOpen, onClose, loanId, onSuccess }: MerchantIssuedRefundModalProps) {
  const [template, setTemplate] = useState<MerchantIssuedRefundTemplate | null>(null);
  ...
  const fetchTemplate = async () => {
    try {
      ...
      console.error("Error fetching merchant issued refund template:", err);
      setError(err instanceof Error ? err.message : "Failed to load merchant issued refund template");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      console.error("Error submitting merchant issued refund:", err);
      setError(err instanceof Error ? err.message : "Failed to submit merchant issued refund");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    return (
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>Merchant Issued Refund</DialogTitle>
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Record a merchant issued refund for loan #{loanId}
          </DialogDescription>
        </DialogHeader>
        ...
        {success && (
          <Alert className="border-green-200 bg-green-50">
            ...
            <AlertDescription className="text-green-800">
              Merchant Issued Refund submitted successfully!
            </AlertDescription>
          </Alert>
        )}
