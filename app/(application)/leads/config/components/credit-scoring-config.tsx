...
import { ScoringFactor } from "@/shared/types/credit-scoring";
...
export function CreditScoringConfig() {
  ...
  const saveConfiguration = async () => {
    ...
    try {
      const response = await fetch("/api/credit-scoring/config", {
        method: "POST",
        body: JSON.stringify(factors),
      });
      if (!response.ok) {
        throw new Error("Failed to save credit scoring configuration");
      }
    } catch (error) {
      console.error("Error saving credit scoring configuration:", error);
    };
    ...
    return (
      <div className="space-y-8">
        ...
        {/* Weight Status Card */}
        <Card
          className={`border-2 ${isValidWeight
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <div
                  className={`text-3xl font-bold ${isValidWeight ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"
                    }`}
                >
                  {totalWeight}%
                </div>
